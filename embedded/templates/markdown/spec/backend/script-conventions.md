...
> Standards for Python scripts in the `.harness-cli/scripts/` directory.
...
def get_repo_root() -> Path | None:
    """Find repository root by looking for .harness-cli directory."""
...
print("Run: python3 ./.harness-cli/scripts/init_developer.py <name>")
...
path = Path(".harness-cli") / "scripts" / "task.py"

# Bad - Unix-only
path = ".harness-cli/scripts/task.py"
```
...
hooks:
  after_create:
    - "python3 .harness-cli/scripts/hooks/my_hook.py create"
  after_start:
    - "python3 .harness-cli/scripts/hooks/my_hook.py start"
  after_archive:
    - "python3 .harness-cli/scripts/hooks/my_hook.py archive"
```
...
1. Store config in a **gitignored** local file (e.g., `.harness-cli/hooks.local.json`)
2. Read config at startup, fail with clear message if missing
3. Keep the script itself committable (no hardcoded secrets)

```python
# .harness-cli/scripts/hooks/my_hook.py — committable, no secrets
CONFIG = _load_config()  # reads from .harness-cli/hooks.local.json (gitignored)
...
Scripts that modify `.harness-cli/` tracked files should auto-commit their changes to keep the workspace clean. Use a `--no-commit` flag for opt-out.

### Convention: Auto-Commit After Mutation

```python
def _auto_commit(scope: str, message: str, repo_root: Path) -> None:
    """Stage and commit changes in a specific .harness-cli/ subdirectory."""
...
- `add_session.py` — commits `.harness-cli/workspace` + `.harness-cli/tasks` after recording a session
- `task.py archive` — commits `.harness-cli/tasks` after archiving a task
...
```yaml
# Auto-detected monorepo packages (written by harness-cli init)
packages:
  cli:
    path: packages/cli
  docs-site:
...
update:
  skip:
    - .claude/commands/hc/my-custom.md
...
# Usage
if not repo_root:
    error("Not in a Harness CLI project (no .harness-cli directory found)")
    sys.exit(1)
```
...
See `.harness-cli/scripts/task.py` for a comprehensive example with:
...
> **Historical Context**: Scripts were migrated from Bash to Python in v0.3.0 for cross-platform compatibility. The old shell scripts are archived in `.harness-cli/scripts-shell-archive/` (if preserved).
