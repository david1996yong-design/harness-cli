...
This project is a **TypeScript CLI tool** using ES modules. The source code follows a **dogfooding architecture** - Harness CLI uses its own configuration files (`.cursor/`, `.claude/`, `.harness-cli/`) as templates for new projects.
...
│   └── workflow.ts      # Creates .harness-cli/ structure
...
.harness-cli/                # Harness CLI workflow (partially dogfooded)
...
├── worktree.yaml        # Worktree config (Harness CLI-specific)
...
Files that are copied directly from Harness CLI project to user projects:

| Source | Destination | Description |
|--------|-------------|-------------|
| `.cursor/` | `.cursor/` | Entire directory copied |
| `.claude/` | `.claude/` | Entire directory copied |
| `.harness-cli/scripts/` | `.harness-cli/scripts/` | All scripts copied |
| `.harness-cli/workflow.md` | `.harness-cli/workflow.md` | Direct copy |
| `.harness-cli/.gitignore` | `.harness-cli/.gitignore` | Direct copy |
| `.harness-cli/workspace/index.md` | `.harness-cli/workspace/index.md` | Direct copy |
...
|----------------|-------------|--------|
| `src/templates/markdown/spec/**/*.md.txt` | `.harness-cli/spec/**/*.md` | User fills with project-specific content |
| `src/templates/markdown/worktree.yaml.txt` | `.harness-cli/worktree.yaml` | Language-agnostic template |
...
├── .harness-cli/          # From project root .harness-cli/ (filtered)
│   ├── scripts/       # All scripts
│   ├── workspace/
...
```typescript
// Get path to .harness-cli/ (works in dev and production)
getHarnessCliSourcePath(): string

// Read file from .harness-cli/
readHarnessCliFile(relativePath: string): string

// Copy directory from .harness-cli/ with executable scripts
copyHarnessCliDir(srcRelativePath: string, destPath: string, options?: { executable?: boolean }): void
```
...
- Update dogfooding sources (`.cursor/`, `.claude/`, `.harness-cli/scripts/`) when making changes
...
await downloadTemplate("gh:mindfold-ai/Harness CLI/marketplace/specs/electron-fullstack", {
  dir: destDir,
  preferOffline: true,
});
```
...
const tempDir = path.join(os.tmpdir(), `harness-cli-template-${Date.now()}`);
