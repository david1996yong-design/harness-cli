...
python3 ./.harness-cli/scripts/get_developer.py

# If not initialized, run:
python3 ./.harness-cli/scripts/init_developer.py <your-name>
# Example: python3 ./.harness-cli/scripts/init_developer.py cursor-agent
```

This creates:
- `.harness-cli/.developer` - Your identity file (gitignored, not committed)
- `.harness-cli/workspace/<your-name>/` - Your personal workspace directory
...
# Or check manually:
python3 ./.harness-cli/scripts/get_developer.py      # Your identity
python3 ./.harness-cli/scripts/task.py list          # Active tasks
...
cat .harness-cli/spec/<package>/<layer>/index.md

# Always read shared guides
cat .harness-cli/spec/guides/index.md
```
...
cat .harness-cli/spec/<package>/<layer>/error-handling.md
cat .harness-cli/spec/<package>/<layer>/conventions.md
...
2. **Follow Standards** - [!] **MUST read `.harness-cli/spec/` guidelines before coding**
...
# Or get JSON format
python3 ./.harness-cli/scripts/get_context.py --json
```
...
cat .harness-cli/spec/guides/cross-layer-thinking-guide.md
```
...
python3 ./.harness-cli/scripts/task.py create "<title>" --slug <task-name>
```
...
- See `.harness-cli/spec/<package>/<layer>/quality-guidelines.md` for package-specific checks
...
```bash
python3 ./.harness-cli/scripts/add_session.py \
  --title "Session Title" \
  --commit "abc1234" \
...
Use `/hc:finish-work` command to run through:
...
python3 ./.harness-cli/scripts/task.py archive <name>  # Archive to archive/{year-month}/
python3 ./.harness-cli/scripts/task.py list            # List active tasks
python3 ./.harness-cli/scripts/task.py list-archive    # List archived tasks
```
...
1. **Before session start**:
   - Run `python3 ./.harness-cli/scripts/get_context.py` for full context
   - [!] **MUST read** relevant `.harness-cli/spec/` docs

2. **During development**:
   - [!] **Follow** `.harness-cli/spec/` guidelines
   - For cross-layer features, use `/hc:check-cross-layer`
   - Develop only one task at a time
   - Run lint and tests frequently

3. **After development complete**:
   - Use `/hc:finish-work` for completion checklist
   - After fix bug, use `/hc:break-loop` for deep analysis
   - Human commits after testing passes
   - Use `add_session.py` to record progress

### [X] DON'T - Should Not Do

1. [!] **Don't** skip reading `.harness-cli/spec/` guidelines
...
```bash
# Session management
python3 ./.harness-cli/scripts/get_context.py    # Get full context
python3 ./.harness-cli/scripts/add_session.py    # Record session

# Task management
python3 ./.harness-cli/scripts/task.py list      # List tasks
python3 ./.harness-cli/scripts/task.py create "<title>" # Create task

# Slash commands
/hc:finish-work          # Pre-commit checklist
/hc:break-loop           # Post-debug analysis
