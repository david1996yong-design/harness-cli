...
after initializing Harness CLI for the first time.
...
Prerequisites:
    - .harness-cli/.developer must exist (run init_developer.py first)

Creates:
    .harness-cli/tasks/00-bootstrap-guidelines/
...
|------|------------------|
| `.harness-cli/{spec_base}/backend/directory-structure.md` | Where different file types go (routes, services, utils) |
| `.harness-cli/{spec_base}/backend/database-guidelines.md` | ORM, migrations, query patterns, naming conventions |
| `.harness-cli/{spec_base}/backend/error-handling.md` | How errors are caught, logged, and returned |
| `.harness-cli/{spec_base}/backend/logging-guidelines.md` | Log levels, format, what to log |
| `.harness-cli/{spec_base}/backend/quality-guidelines.md` | Code review standards, testing requirements |
"""
...
| File | What to Document |
|------|------------------|
| `.harness-cli/{spec_base}/frontend/directory-structure.md` | Component/page/hook organization |
| `.harness-cli/{spec_base}/frontend/component-guidelines.md` | Component patterns, props conventions |
| `.harness-cli/{spec_base}/frontend/hook-guidelines.md` | Custom hook naming, patterns |
| `.harness-cli/{spec_base}/frontend/state-management.md` | State library, patterns, what goes where |
| `.harness-cli/{spec_base}/frontend/type-safety.md` | TypeScript conventions, type organization |
| `.harness-cli/{spec_base}/frontend/quality-guidelines.md` | Linting, testing, accessibility |
"""
...
def write_task_json(task_dir: Path, developer: str, project_type: str, spec_base: str) -> None:
    ...
    if project_type == "frontend":
        ...
        related_files = [f".harness-cli/{spec_base}/frontend/"]
    elif project_type == "backend":
        subtasks = [
            {"name": "Fill backend guidelines", "status": "pending"},
            {"name": "Add code examples", "status": "pending"},
        ]
        related_files = [f".harness-cli/{spec_base}/backend/"]
    else:  # fullstack
        subtasks = [
            {"name": "Fill backend guidelines", "status": "pending"},
            {"name": "Fill frontend guidelines", "status": "pending"},
        ...
        related_files = [f".harness-cli/{spec_base}/backend/", f".harness-cli/{spec_base}/frontend/"]
    ...
    task_data = {
        ...
        "notes": f"First-time setup task created by harness-cli init ({project_type} project)",
        "meta": {},
    }
