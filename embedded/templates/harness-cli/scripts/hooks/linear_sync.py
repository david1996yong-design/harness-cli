#!/usr/bin/env python3
"""Linear sync hook for Harness CLI task lifecycle.

Syncs task events to Linear via the `linearis` CLI.

Usage (called automatically by task.py hooks):
    python3 .harness-cli/scripts/hooks/linear_sync.py create
    python3 .harness-cli/scripts/hooks/linear_sync.py start
    python3 .harness-cli/scripts/hooks/linear_sync.py archive

Manual usage:
    TASK_JSON_PATH=.harness-cli/tasks/<name>/task.json python3 .harness-cli/scripts/hooks/linear_sync.py sync

Environment:
    TASK_JSON_PATH  - Absolute path to task.json (set by task.py)

Configuration:
    .harness-cli/hooks.local.json  - Local config (gitignored), example:
    {
      "linear": {
        "team": "TEAM_KEY",
...
# Harness CLI priority → Linear priority (1=Urgent, 2=High, 3=Medium, 4=Low)
...
def _load_config() -> dict:
    """Load local hook config from .harness-cli/hooks.local.json."""
    task_json_path = os.environ.get("TASK_JSON_PATH", "")
    if task_json_path:
        # Walk up from task.json to find .harness-cli/
        harness_cli_dir = Path(task_json_path).parent.parent.parent
    else:
        harness_cli_dir = Path(".harness-cli")

    config_path = harness_cli_dir / "hooks.local.json"
    try:
