...
class CLIAdapter:
    ...
    _AGENT_NAME_MAP: ClassVar[dict[Platform, dict[str, str]]] = {
        ...
        "opencode": {
            "plan": "hc-plan",  # 'plan' is built-in in OpenCode
        },
    }
    ...
    def get_agent_name(self, agent: str) -> str:
        ...
        Returns:
            Platform-specific agent name (e.g., 'hc-plan' for OpenCode)
        """
    ...
    def get_commands_path(self, project_root: Path, *parts: str) -> Path:
        ...
        Args:
            project_root: Project root directory
            *parts: Additional path parts (e.g., 'harness-cli', 'finish-work.md')

        Returns:
            Path to commands directory or file

        Note:
            Cursor uses prefix naming: .cursor/commands/harness-cli-<name>.md
            Antigravity uses workflow directory: .agent/workflows/<name>.md
            Windsurf uses workflow directory: .windsurf/workflows/harness-cli-<name>.md
            Copilot uses prompt files: .github/prompts/<name>.prompt.md
            Claude/OpenCode use subdirectory: .claude/commands/hc/<name>.md
        """
        if self.platform == "windsurf":
            workflow_dir = self.get_config_dir(project_root) / "workflows"
            if not parts:
                return workflow_dir
            if len(parts) >= 2 and parts[0] == "harness-cli":
                filename = parts[-1]
                return workflow_dir / f"harness-cli-{filename}"
        ...
        if self.platform == "cursor" and len(parts) >= 2 and parts[0] == "harness-cli":
            # Convert harness-cli/<name>.md to harness-cli-<name>.md
            filename = parts[-1]
            return (
                self.get_config_dir(project_root) / "commands" / f"harness-cli-{filename}"
            )

        return self.get_config_dir(project_root) / "commands" / Path(*parts)

    def get_harness_cli_command_path(self, name: str) -> str:
        """Get relative path to a harness-cli command file.
        ...
        Note:
            ...
            Gemini: .gemini/commands/hc/<name>.toml
            Antigravity: .agent/workflows/<name>.md
            Windsurf: .windsurf/workflows/harness-cli-<name>.md
            Others: .{platform}/commands/hc/<name>.md
        """
        if self.platform == "cursor":
            return f".cursor/commands/harness-cli-{name}.md"
        elif self.platform == "codex":
            return f".agents/skills/{name}/SKILL.md"
        elif self.platform == "kiro":
            return f".kiro/skills/{name}/SKILL.md"
        elif self.platform == "gemini":
            return f".gemini/commands/hc/{name}.toml"
        elif self.platform == "antigravity":
            return f".agent/workflows/{name}.md"
        elif self.platform == "windsurf":
            return f".windsurf/workflows/harness-cli-{name}.md"
        elif self.platform == "kilo":
            return f".kilocode/workflows/{name}.md"
        elif self.platform == "copilot":
            return f".github/prompts/{name}.prompt.md"
        else:
            return f"{self.config_dir_name}/commands/hc/{name}.md"
...
def detect_platform(project_root: Path) -> Platform:
    ...
    Detection order:
    1. HARNESS_CLI_PLATFORM environment variable (if set)
    ...
    env_platform = os.environ.get("HARNESS_CLI_PLATFORM", "").lower()
    if env_platform in (
        "claude",
        "opencode",
        "cursor",
