#!/usr/bin/env python3
"""
Common path utilities for Harness CLI workflow.
...
def get_repo_root(start_path: Path | None = None) -> Path:
    """Find the nearest directory containing .harness-cli/ folder.
    ...
    Returns:
        Path to repository root, or current directory if no .harness-cli/ found.
    """
    ...
    # Fallback to current directory if no .harness-cli/ found
...
def normalize_task_ref(task_ref: str) -> str:
    ...
    `.harness-cli/tasks/03-27-my-task`, even on Windows. Absolute paths are preserved
...
def get_spec_dir(package: str | None = None, repo_root: Path | None = None) -> Path:
    ...
    Single-repo: .harness-cli/spec
    Monorepo with package: .harness-cli/spec/<package>
