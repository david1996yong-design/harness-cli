...
Example: python3 start.py .harness-cli/tasks/01-21-my-task
...
Configuration: .harness-cli/worktree.yaml
"""
...
def main() -> int:
    ...
    cli_cmd = adapter.build_run_command(
        agent="dispatch",
        prompt="Follow your agent instructions to execute the task workflow. Start by reading .harness-cli/.current-task to get the task directory, then execute each action in task.json next_action array in order.",
