...
def _auto_commit_workspace(repo_root: Path) -> None:
    """Stage .harness-cli/workspace and .harness-cli/tasks, then commit with a configured message."""
    commit_msg = get_session_commit_message(repo_root)
    subprocess.run(
        ["git", "add", "-A", ".harness-cli/workspace", ".harness-cli/tasks"],
        cwd=repo_root,
        capture_output=True,
    )
    # Check if there are staged changes
    result = subprocess.run(
        ["git", "diff", "--cached", "--quiet", "--", ".harness-cli/workspace", ".harness-cli/tasks"],
        cwd=repo_root,
    )
