...
#
# Usage:
#   ./.harness-cli/scripts/create-bootstrap.sh [project-type]
#
# Arguments:
#   project-type: frontend | backend | fullstack (default: fullstack)
#
# Prerequisites:
#   - .harness-cli/.developer must exist (run init-developer.sh first)
#
# Creates:
...
```bash
./.harness-cli/scripts/task.sh finish
./.harness-cli/scripts/task.sh archive 00-bootstrap-guidelines
```
  ...
  cat > "$dir/task.json" << EOF
{
  ...
  "notes": "First-time setup task created by harness-cli init ($project_type project)"
}
EOF
}
