...
#
# Usage:
#   ./.harness-cli/scripts/task.sh create "<title>" [--slug <name>] [--assignee <dev>] [--priority P0|P1|P2|P3]
#   ./.harness-cli/scripts/task.sh init-context <dir> <type>   # Initialize jsonl files
#   ./.harness-cli/scripts/task.sh add-context <dir> <file> <path> [reason] # Add jsonl entry
#   ./.harness-cli/scripts/task.sh validate <dir>              # Validate jsonl files
#   ./.harness-cli/scripts/task.sh list-context <dir>          # List jsonl entries
#   ./.harness-cli/scripts/task.sh start <dir>                 # Set as current task
#   ./.harness-cli/scripts/task.sh finish                      # Clear current task
#   ./.harness-cli/scripts/task.sh set-branch <dir> <branch>   # Set git branch
#   ./.harness-cli/scripts/task.sh set-scope <dir> <scope>     # Set scope for PR title
#   ./.harness-cli/scripts/task.sh create-pr [dir] [--dry-run] # Create PR from task
#   ./.harness-cli/scripts/task.sh archive <task-name>         # Archive completed task
#   ./.harness-cli/scripts/task.sh list                        # List active tasks
#   ./.harness-cli/scripts/task.sh list-archive [month]        # List archived tasks
#
...
# Claude: .claude/commands/hc/<name>.md
# Cursor: .cursor/commands/harness-cli-<name>.md
get_command_path() {
  local name="$1"
  if [[ "$PLATFORM" == "cursor" ]]; then
    echo ".cursor/commands/harness-cli-${name}.md"
  else
    echo ".claude/commands/hc/${name}.md"
  fi
}
...
cmd_set_branch() {
  ...
  echo "  ./.harness-cli/scripts/multi-agent/start.sh $1"
}
...
Examples:
  ...
  $0 init-context .harness-cli/tasks/01-21-add-login backend
  $0 init-context .harness-cli/tasks/01-21-add-login frontend --platform cursor
  $0 add-context <dir> implement .harness-cli/spec/backend/auth.md "Auth guidelines"
  $0 set-branch <dir> task/add-login
  $0 start .harness-cli/tasks/01-21-add-login
