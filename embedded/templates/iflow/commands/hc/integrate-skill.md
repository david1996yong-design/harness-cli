# Integrate iFlow Skill into Project Guidelines

Adapt and integrate a iFlow global skill into your project's development guidelines (not directly into project code).

## Usage

```
/hc:integrate-skill <skill-name>
```

**Examples**:
```
/hc:integrate-skill frontend-design
/hc:integrate-skill mcp-builder
```
...
>
> - Guidelines content -> Write to `.harness-cli/spec/{target}/doc.md`
> - Code examples -> Place in `.harness-cli/spec/{target}/examples/skills/<skill-name>/`
...
|----------------|-------------------|
| UI/Frontend (`frontend-design`, `web-artifacts-builder`) | `.harness-cli/spec/frontend/` |
| Backend/API (`mcp-builder`) | `.harness-cli/spec/backend/` |
| Documentation (`doc-coauthoring`, `docx`, `pdf`) | `.harness-cli/` or create dedicated guidelines |
| Testing (`webapp-testing`) | `.harness-cli/spec/frontend/` (E2E) |
...
- **Integration target**: `.harness-cli/spec/{target}/`
...
| Type | Path |
|------|------|
| Guidelines doc | `.harness-cli/spec/{target}/doc.md` (section: `skill-<name>`) |
| Code examples | `.harness-cli/spec/{target}/examples/skills/<name>/` |
| Index update | `.harness-cli/spec/{target}/index.md` |
...
```bash
/hc:create-command use-<skill-name> Use <skill-name> skill following project guidelines
```
...
| `doc-coauthoring` | `.harness-cli/` | N/A (documentation workflow only) |
