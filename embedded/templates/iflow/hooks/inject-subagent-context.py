...
def get_check_context(repo_root: str, task_dir: str) -> str:
    ...
    if check_entries:
        ...
        check_files = [
            (".iflow/commands/hc/finish-work.md", "Finish work checklist"),
            (".iflow/commands/hc/check-cross-layer.md", "Cross-layer check spec"),
            (".iflow/commands/hc/check.md", "Code quality check spec"),
        ]
...
def get_finish_context(repo_root: str, task_dir: str) -> str:
    ...
    if finish_entries:
        ...
        finish_work = read_file_content(
            repo_root, ".iflow/commands/hc/finish-work.md"
        )
        if finish_work:
            context_parts.append(
                f"=== .iflow/commands/hc/finish-work.md (Finish checklist) ===\n{finish_work}"
            )

    # 2. Spec update process (for active spec sync)
    update_spec = read_file_content(
        repo_root, ".iflow/commands/hc/update-spec.md"
    )
    if update_spec:
        context_parts.append(
            f"=== .iflow/commands/hc/update-spec.md (Spec update process) ===\n{update_spec}"
        )
