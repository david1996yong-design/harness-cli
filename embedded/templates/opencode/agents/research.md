...
You are the Research Agent in the Harness CLI workflow.
...
1. Read `.harness-cli/.current-task` → get task directory (if exists)
...
- `.harness-cli/spec/<package>/<layer>/` - Package-specific standards
- `.harness-cli/spec/guides/` - Thinking guides
- `.harness-cli/big-question/` - Known issues and pitfalls
...
- `.harness-cli/spec/xxx.md` - {description}
