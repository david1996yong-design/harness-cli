...
2. Run with revised requirement:
   ```bash
   python3 ./.harness-cli/scripts/multi_agent/plan.py --name "<name>" --type "<type>" --requirement "<revised requirement>"
   ```
```
...
```bash
python3 ./.harness-cli/scripts/task.py init-context "$PLAN_TASK_DIR" "$PLAN_DEV_TYPE"
```
...
Instructions:
1. Search .harness-cli/spec/ for relevant spec files
...
python3 ./.harness-cli/scripts/task.py add-context "$PLAN_TASK_DIR" implement "<path>" "<reason>"

# For each entry in check.jsonl section:
python3 ./.harness-cli/scripts/task.py add-context "$PLAN_TASK_DIR" check "<path>" "<reason>"

# For each entry in debug.jsonl section:
python3 ./.harness-cli/scripts/task.py add-context "$PLAN_TASK_DIR" debug "<path>" "<reason>"
```
...
```bash
# Set branch name
python3 ./.harness-cli/scripts/task.py set-branch "$PLAN_TASK_DIR" "feature/${PLAN_TASK_NAME}"

# Set scope (from research agent suggestion)
python3 ./.harness-cli/scripts/task.py set-scope "$PLAN_TASK_DIR" "<scope>"
...
```bash
python3 ./.harness-cli/scripts/task.py validate "$PLAN_TASK_DIR"
```
...
python3 ./.harness-cli/scripts/task.py list-context "$PLAN_TASK_DIR"
echo ""
echo "Ready for: python3 ./.harness-cli/scripts/multi_agent/start.py $PLAN_TASK_DIR"
```
...
Output:
  .harness-cli/tasks/02-03-add-rate-limiting/
