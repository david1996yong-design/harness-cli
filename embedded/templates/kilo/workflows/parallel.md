...
```bash
python3 ./.harness-cli/scripts/get_context.py --mode packages  # Discover available spec layers
cat .harness-cli/spec/guides/index.md    # Thinking guides
```
...
```bash
python3 ./.harness-cli/scripts/multi_agent/plan.py \
  --name "<feature-name>" \
...
```bash
python3 ./.harness-cli/scripts/multi_agent/start.py "$TASK_DIR" --platform opencode

# Or with direct merge (skip PR):
python3 ./.harness-cli/scripts/multi_agent/start.py "$TASK_DIR" --platform opencode --merge
python3 ./.harness-cli/scripts/multi_agent/start.py "$TASK_DIR" --platform opencode --merge master  # explicit target
```
...
TASK_DIR=$(python3 ./.harness-cli/scripts/task.py create "<title>" --slug <task-name>)
```
...
python3 ./.harness-cli/scripts/task.py init-context "$TASK_DIR" <dev_type>

# Set branch and scope
python3 ./.harness-cli/scripts/task.py set-branch "$TASK_DIR" feature/<name>
python3 ./.harness-cli/scripts/task.py set-scope "$TASK_DIR" <scope>
```
...
| Command | Description |
|---------|-------------|
| `/hc:parallel` | Start Multi-Agent Pipeline (this command) |
| `/hc:start` | Start normal development mode (single process) |
| `/hc:record-session` | Record session progress |
| `/hc:finish-work` | Pre-completion checklist |
...
```bash
python3 ./.harness-cli/scripts/multi_agent/status.py                    # Overview
python3 ./.harness-cli/scripts/multi_agent/status.py --log <name>       # View log
python3 ./.harness-cli/scripts/multi_agent/status.py --watch <name>     # Real-time monitoring
python3 ./.harness-cli/scripts/multi_agent/cleanup.py <branch>          # Cleanup worktree
```
