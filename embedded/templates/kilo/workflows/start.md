...
|------------|------|--------|
| `/hc:start` | Dev | Proceed to **Step 5: Report and Ask** |
| `/hc:start dev` | Dev | Proceed to **Step 5: Report and Ask** |
...
See `/hc:brainstorm` for the full process. Summary:
...
| Command | When to Use |
|---------|-------------|
| `/hc:start` | Begin a dev session (default mode) |
| `/hc:start debug [desc]` | Begin a debug diagnosis session |
| `/hc:start arch [module]` | Begin an architecture review session |
| `/hc:parallel` | Complex tasks needing isolated worktree |
| `/hc:finish-work` | Before committing changes |
| `/hc:record-session` | After completing a task |
...
| `python3 ./.harness-cli/scripts/task.py add-context` | Add spec to jsonl |
...
3. **Call Check Agent** to verify:
   ...
   Task(
     subagent_type: "check",
     prompt: "Review the debug fix against the specs.
