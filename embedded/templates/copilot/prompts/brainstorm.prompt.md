---
description: "Harness CLI Copilot prompt: Brainstorm - Requirements Discovery (AI Coding Enhanced)"
---
...
Triggered from `/hc:start` when the user describes a development task, especially when:
...
```bash
# Create child tasks
CHILD1=$(python3 ./.harness-cli/scripts/task.py create "Child task 1" --slug child1 --parent "$TASK_DIR")
CHILD2=$(python3 ./.harness-cli/scripts/task.py create "Child task 2" --slug child2 --parent "$TASK_DIR")

# Or link existing tasks
python3 ./.harness-cli/scripts/task.py add-subtask "$TASK_DIR" "$CHILD_DIR"
```
...
| Command | When to Use |
|---------|-------------|
| `/hc:start` | Entry point that triggers brainstorm |
| `/hc:finish-work` | After implementation is complete |
| `/hc:update-spec` | If new patterns emerge during work |
