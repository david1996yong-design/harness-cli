...
Current Task is specified by `.harness-cli/.current-task` file, content is the relative path to task directory.

Task directory path format: `.harness-cli/tasks/{MM}-{DD}-{name}/`
...
Read `.harness-cli/.current-task` to get current task directory path:

```bash
TASK_DIR=$(cat .harness-cli/.current-task)
# e.g.: .harness-cli/tasks/02-03-my-feature
```
