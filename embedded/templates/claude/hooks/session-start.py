...
def _normalize_task_ref(task_ref: str) -> str:
    ...
    if normalized.startswith("tasks/"):
        return f".harness-cli/{normalized}"

    return normalized


def _resolve_task_dir(harness_cli_dir: Path, task_ref: str) -> Path:
    normalized = _normalize_task_ref(task_ref)
    path_obj = Path(normalized)
    if path_obj.is_absolute():
        return path_obj
    if normalized.startswith(".harness-cli/"):
        return harness_cli_dir.parent / path_obj
    return harness_cli_dir / "tasks" / path_obj


def _get_task_status(harness_cli_dir: Path) -> str:
    """Check current task status and return structured status string."""
    current_task_file = harness_cli_dir / ".current-task"
    ...
    task_dir = _resolve_task_dir(harness_cli_dir, task_ref)
    if not task_dir.is_dir():
        return f"Status: STALE POINTER\nTask: {task_ref}\nNext: Task directory not found. Run: python3 ./.harness-cli/scripts/task.py finish"
    ...
    if task_status == "completed":
        return f"Status: COMPLETED\nTask: {task_title}\nNext: Archive with `python3 ./.harness-cli/scripts/task.py archive {task_dir.name}` or start a new task"
...
def _load_harness_cli_config(harness_cli_dir: Path) -> tuple:
    """Load Harness CLI config for session-start decisions.

    Returns:
        (is_mono, packages_dict, spec_scope, task_pkg, default_pkg)
    """
    scripts_dir = harness_cli_dir / "scripts"
    ...
    try:
        ...
        repo_root = harness_cli_dir.parent
...
def _check_legacy_spec(harness_cli_dir: Path, is_mono: bool, packages: dict) -> str | None:
...
def main():
    ...
    harness_cli_dir = project_dir / ".harness-cli"
    claude_dir = project_dir / ".claude"

    # Load config for scope filtering and legacy detection
    is_mono, packages, scope_config, task_pkg, default_pkg = _load_harness_cli_config(harness_cli_dir)
    allowed_pkgs = _resolve_spec_scope(is_mono, packages, scope_config, task_pkg, default_pkg)

    output = StringIO()

    output.write("""<session-context>
You are starting a new session in a Harness CLI-managed project.
...
""")
    ...
    legacy_warning = _check_legacy_spec(harness_cli_dir, is_mono, packages)
    if legacy_warning:
        output.write(f"<migration-warning>\n{legacy_warning}\n</migration-warning>\n\n")

    output.write("<current-state>\n")
    context_script = harness_cli_dir / "scripts" / "get_context.py"
    output.write(run_script(context_script))
    output.write("\n</current-state>\n\n")

    output.write("<workflow>\n")
    workflow_content = read_file(harness_cli_dir / "workflow.md", "No workflow.md found")
    ...
    start_md = read_file(
        claude_dir / "commands" / "harness-cli" / "start.md", "No start.md found"
    )
    output.write(start_md)
    output.write("\n</instructions>\n\n")

    # Check task status and inject structured tag
    task_status = _get_task_status(harness_cli_dir)
