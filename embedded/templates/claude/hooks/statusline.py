...
# -*- coding: utf-8 -*-
"""
Harness CLI StatusLine — project-level status display for Claude Code.

Reads Claude Code session JSON from stdin + Harness CLI task data from filesystem.
Outputs 1-2 lines:
...
def _find_harness_cli_dir() -> Path | None:
    """Walk up from cwd to find .harness-cli/ directory."""
    current = Path.cwd()
    for parent in [current, *current.parents]:
        candidate = parent / ".harness-cli"
        if candidate.is_dir():
            return candidate
    return None


def _get_current_task(harness_cli_dir: Path) -> dict | None:
    """Load current task info. Returns dict with title/status/priority or None."""
    task_ref = _normalize_task_ref(_read_text(harness_cli_dir / ".current-task"))
    if not task_ref:
        return None

    # Resolve task directory
    task_path = _resolve_task_dir(harness_cli_dir, task_ref)
...
def _count_active_tasks(harness_cli_dir: Path) -> int:
    """Count non-archived task directories with valid task.json."""
    tasks_dir = harness_cli_dir / "tasks"
...
def _get_developer(harness_cli_dir: Path) -> str:
    content = _read_text(harness_cli_dir / ".developer")
    if not content:
        return "unknown"
...
def main() -> None:
    ...
    harness_cli_dir = _find_harness_cli_dir()
    SEP = " \033[90m·\033[0m "

    # --- Harness CLI data ---
    task = _get_current_task(harness_cli_dir) if harness_cli_dir else None
    dev = _get_developer(harness_cli_dir) if harness_cli_dir else ""
    task_count = _count_active_tasks(harness_cli_dir) if harness_cli_dir else 0
