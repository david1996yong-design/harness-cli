...
Context Source: .harness-cli/.current-task points to task directory
...
def get_current_task(repo_root: str) -> str | None:
    """
    Read current task directory path from .harness-cli/.current-task
    ...
    try:
        with open(current_task_file, "r", encoding="utf-8") as f:
            ...
            if normalized.startswith("tasks/"):
                normalized = f".harness-cli/{normalized}"
            return normalized
    except Exception:
        return None
...
def get_check_context(repo_root: str, task_dir: str) -> str:
    ...
    if check_entries:
        ...
        check_files = [
            (".claude/commands/hc/finish-work.md", "Finish work checklist"),
            (".claude/commands/hc/check-cross-layer.md", "Cross-layer check spec"),
            (".claude/commands/hc/check.md", "Code quality check spec"),
        ]
...
def get_finish_context(repo_root: str, task_dir: str) -> str:
    ...
    if finish_entries:
        ...
        finish_work = read_file_content(
            repo_root, ".claude/commands/hc/finish-work.md"
        )
        if finish_work:
            context_parts.append(
                f"=== .claude/commands/hc/finish-work.md (Finish checklist) ===\n{finish_work}"
            )

    # 2. Spec update process (for active spec sync)
    update_spec = read_file_content(
        repo_root, ".claude/commands/hc/update-spec.md"
    )
    if update_spec:
        context_parts.append(
            f"=== .claude/commands/hc/update-spec.md (Spec update process) ===\n{update_spec}"
        )
