...
|---------|-------------|
| `/hc-start` | Start development session (this workflow) |
...
Harness CLI auto-detects your platform based on config directories. For Windsurf users, ensure detection works correctly:
...
```bash
export HARNESS_CLI_PLATFORM=windsurf
```

Or prefix commands:

```bash
HARNESS_CLI_PLATFORM=windsurf python3 ./.harness-cli/scripts/task.py list
```
