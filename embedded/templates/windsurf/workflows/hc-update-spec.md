...
```bash
cat .harness-cli/spec/<category>/<file>.md
```
...
```
Development Flow:
  Learn something → /harness-cli-update-spec → Knowledge captured
       ↑                                  ↓
  /hc-break-loop ←──────────────────── Future sessions benefit
  (deep bug analysis)
```

- `/hc-break-loop` - Analyzes bugs deeply, often reveals spec updates needed
- `/harness-cli-update-spec` - Actually makes the updates (this workflow)
- `/hc-finish-work` - Reminds you to check if specs need updates
