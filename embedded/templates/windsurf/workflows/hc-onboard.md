...
**The Solution**: The `.harness-cli/workspace/` system captures what happened in each session - what was done, what was learned, what problems were solved. The `/hc-start` workflow reads this history at session start, giving AI "artificial memory."
...
**The Solution**: The `.harness-cli/spec/` directory contains project-specific guidelines. The `/hc-before-dev` workflow injects this specialized knowledge into AI context before coding starts.
...
**The Solution**: The `/hc-check-*` workflows re-verify code against guidelines AFTER writing, catching drift that occurred during development. The `/hc-finish-work` workflow does a final holistic review.
...
### /hc-before-dev - Inject Specialized Knowledge
...
**WHY IT EXISTS**:
The `/hc-check-*` workflows focus on code quality within a single layer. But real changes often have cross-cutting concerns.
...
**[2/4] python3 ./.harness-cli/scripts/task.py create "Planning task" --slug planning-task** - Planning is valuable work
...
**[4/5] /hc-finish-work** - Check if new patterns should be documented
**[5/5] Record with multiple commit hashes** - Link all commits to one feature

### Example 5: Debug Session

**[1/6] /hc-start** - See if this bug was investigated before
**[2/6] /hc-before-dev** - Guidelines might document known gotchas
**[3/6] Investigation** - Actual debugging work
**[4/6] /hc-check** - Verify debug changes do not break other things
**[5/6] /hc-finish-work** - Debug findings might need documentation
**[6/6] Human commits, then /hc-record-session** - Debug knowledge is valuable
...
2. **Guidelines before code** - /hc-before-dev workflow injects project knowledge.
3. **Check after code** - /hc-check-* workflows catch context drift.
...
If guidelines are empty templates (contain "To be filled by the team"), this is the first time using Harness CLI in this project.

Explain to the developer:

"I see that the development guidelines in `.harness-cli/spec/` are still empty templates. This is normal for a new Harness CLI setup!

The templates contain placeholder text that needs to be replaced with YOUR project's actual conventions. Without this, the `/hc-before-dev` workflow won't provide useful guidance.
...
For example, for `.harness-cli/spec/backend/database-guidelines.md`:
...
"Great! Your team has already customized the development guidelines. You can start using the `/hc-before-dev` workflow right away.

I recommend reading through `.harness-cli/spec/` to familiarize yourself with the team's coding standards."
...
```bash
python3 ./.harness-cli/scripts/task.py create "Fill spec guidelines" --slug fill-spec-guidelines
```
...
2. [If guidelines empty] Start filling in `.harness-cli/spec/` guidelines
