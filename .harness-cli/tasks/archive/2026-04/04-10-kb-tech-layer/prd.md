...
1. 确认 `kb/tech/` 目录存在
2. 读取模板
3. 分析项目结构——但关注架构维度而非业务维度：
   - 技术栈识别（语言、框架、构建工具、依赖）
   - 组件边界和依赖关系
   - 核心数据结构 / Schema
   - 跨模块的共享基础设施（工具函数、中间件、配置）
...
更新 `kb/prd/index.md` 模板，在概述的三层表中加入 tech：
...
* 代码风格与现有 scan.rs / markdown.rs 一致
* 模板格式与 kb/prd/ 保持一致
* 中文撰写

## Out of Scope

* `infrastructure.md`（不做）
* 不做 update-kb-tech 增量更新（后续任务）
* 不修改其他 AI 平台的命令（Cursor 等）

## Technical Notes

* `scan.rs`: `src/commands/scan.rs` — 新增 kb/tech 脚手架
* `markdown.rs`: `src/templates/markdown.rs` — 新增 tech 模板访问器
* `paths.rs`: `src/constants/paths.rs` — 新增 KB_TECH 常量
* `scan-kb.md`: `embedded/templates/claude/commands/hc/scan-kb.md` — 参考格式
* `kb/prd/index.md.txt`: `embedded/templates/markdown/kb/prd/index.md.txt` — 参考格式
* `kb/prd/module-template.md.txt`: `embedded/templates/markdown/kb/prd/module-template.md.txt` — 参考格式
