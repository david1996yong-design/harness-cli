//! CodeBuddy templates.
//!
//! CodeBuddy uses nested directories: `commands/hc/<name>.md`.

use super::extract::{get_embedded_file, list_files, CodeBuddyTemplates};
...
pub fn get_all_commands() -> Vec<CommandTemplate> {
    ...
    for path in list_files::<CodeBuddyTemplates>() {
        if path.starts_with("commands/hc/") && path.ends_with(".md") {
            if let Some(content) = get_embedded_file::<CodeBuddyTemplates>(&path) {
                let name = path
...
mod tests {
    ...
    fn test_get_all_commands_non_empty() {
        ...
        assert!(
            !commands.is_empty(),
            "CodeBuddy commands should be non-empty"
        );
    }
