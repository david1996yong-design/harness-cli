...
//!
//! Windsurf uses `workflows/*.md` with `hc-` prefix naming.

use super::extract::{get_embedded_file, list_files, WindsurfTemplates};
...
pub fn get_all_workflows() -> Vec<WorkflowTemplate> {
    ...
    for path in list_files::<WindsurfTemplates>() {
        if path.starts_with("workflows/") && path.ends_with(".md") {
            if let Some(content) = get_embedded_file::<WindsurfTemplates>(&path) {
                let name = path
...
mod tests {
    ...
    fn test_get_all_workflows_non_empty() {
        ...
        assert!(
            !workflows.is_empty(),
            "Windsurf workflows should be non-empty"
        );
    }
