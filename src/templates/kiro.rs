...
//!
//! Kiro uses `skills/<name>/SKILL.md`.

use super::extract::{get_embedded_file, list_files, KiroTemplates};
...
pub fn get_all_skills() -> Vec<SkillTemplate> {
    ...
    for path in list_files::<KiroTemplates>() {
        if path.starts_with("skills/") && path.ends_with("/SKILL.md") {
            if let Some(content) = get_embedded_file::<KiroTemplates>(&path) {
                let name = path
                    .strip_prefix("skills/")
                    .unwrap()
...
mod tests {
    ...
    fn test_get_all_skills_non_empty() {
        ...
        assert!(!skills.is_empty(), "Kiro skills should be non-empty");
    }
